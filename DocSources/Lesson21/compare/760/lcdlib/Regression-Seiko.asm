...
TstAd1		movf		Index,W		; Pick up the index
            ...
            addlw       H'44'       ; Center on bottom line
...
; Use both lines of 20 character display

TstA20
			lcall		LCDclear	; First clear memory
			clrf		Index		; Start with zeroth character
TstA201		movf		Index,W		; Pick up the index
			lcall		Tab20_1		; Look up the desired character
			lcall		LCDletr		; Display it
			incf		Index,1		; Next character
			movlw		.20			; Message length
			subwf		Index,W		; WIll be zero when done
			btfss		STATUS,Z	; Zero?
			goto		TstA201		; No, do it again

            movlw       H'40'
            lcall       LCDaddr
			clrf		Index		; Start with zeroth character
TstA202		movf		Index,W		; Pick up the index
			lcall		Tab20_2		; Look up the desired character
			lcall		LCDletr		; Display it
			incf		Index,1		; Next character
			movlw		.20			; Message length
			subwf		Index,W		; WIll be zero when done
			btfss		STATUS,Z	; Zero?
			goto		TstA202		; No, do it again
...
Tab20_1
    IF PROC != 54
    		movwf		SavIdx		; Save off the index
			movlw		HIGH(Tab20_1); Get this page's high byte
			movwf		PCLATH		; and store it into PCLATH
			movf		SavIdx,W	; Pick up index
    ENDIF
			addwf		PCL,F		; And look it up
			dt			"14.060.000 AN 35 Wpm"
;	Table with message for TstA20 line 2
Tab20_2
    IF PROC != 54
    		movwf		SavIdx		; Save off the index
			movlw		HIGH(Tab20_2); Get this page's high byte
			movwf		PCLATH		; and store it into PCLATH
			movf		SavIdx,W	; Pick up index
    ENDIF
			addwf		PCL,F		; And look it up
			dt			"RIT+000 f500    2.5W"
			end
